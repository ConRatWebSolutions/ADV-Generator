...
class EmailConfig {
    ...
    public function sendAgreementToUser(string $userEmail, string $userName, string $pdfPath, string $companyName): bool {
        try {
            ...
            if (!empty($pdfPath) && file_exists($pdfPath)) {
                ...
                error_log("PDF-Datei nicht gefunden: " . $pdfPath);
            }
    ...
    public function sendAgreementToAdmin(string $pdfPath, array $userData): bool {
        try {
            ...
            error_log("Failed to send email to admin: " . $e->getMessage());
            return false;
        }
    }
    ...
    private function sendUserEmail(int $agreementId, array $userData, string $pdfPath): array {
        try {
            ...
            if ($result) {
                ...
                $this->logEmail($agreementId, $userData['email'], 'kunde', 'fehler', 'Send failed');
                return ['success' => false, 'error' => 'Send failed'];
            }
            
        } catch (Exception $e) {
            $this->logEmail($agreementId, $userData['email'], 'kunde', 'fehler', $e->getMessage());
    ...
    private function sendAdminEmail(int $agreementId, array $userData, string $pdfPath): array {
        try {
            ...
            if ($result) {
                ...
                $this->logEmail($agreementId, $this->config['admin_email'], 'admin', 'fehler', 'Send failed');
                return ['success' => false, 'error' => 'Send failed'];
            }
            
        } catch (Exception $e) {
            $this->logEmail($agreementId, $this->config['admin_email'], 'admin', 'fehler', $e->getMessage());
            return ['success' => false, 'error' => $e->getMessage()];
        }
    }
